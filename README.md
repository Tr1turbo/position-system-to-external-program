...
Classic light decoding preserves both established DPS root/front forms. Nominal root ranges `0.41` (hole) and `0.42` (ring) pair with a front marker near `0.45`; nominal root ranges `0.43` (hole) and `0.44` (ring) pair with a front marker near `0.46`. SPS1 forms are identified from complete range values such as `0.4102`, `0.4202`, and `0.4502`. SPS2 compatibility forms are identified from VRCFury's values `0.4106`, `0.4206`, and `0.4506`. Other accepted range variations use the `ClassicLight` DPS-compatible fallback category; `ClassicLight` does not identify one specific producer. Roots pair only with front markers from the same source category and within `0.3 m` of each other in world space; the separation need not equal an exact authored distance. Ordinary DPS-compatible rings map to one-sided `OneWayRing`; SPS1 and SPS2 double-sided ring compatibility data maps to `Ring`.

The ordinary encoder permits all three recognized light families. The SPS2 atlas encoder also permits all three when no valid SPS2 atlas socket is available, preserving SPS2 compatibility lights as a legacy fallback. When at least one valid SPS2 atlas socket is available, the encoder excludes all `Sps2CompatibilityLight` roots and fronts so they cannot compete with their authoritative atlas representation or provide orientation to another family. An atlas plug by itself does not suppress compatibility socket lights. The nearest eligible socket wins; an exact tie may prefer the richer atlas record.
